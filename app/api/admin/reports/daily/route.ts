...
export async function GET(req: Request) {
  ...
  const { data, error } = await supabaseAdmin
    .from("bookings")
    .select("id, start_at, deposit_amount, payment_status, status, customers(name,phone), services(name)")
  ...
  const rows = [
    ["booking_id","datetime","customer","phone","service","deposit","status","payment_status"],
    ...data.map((b:any) => [
      b.id,
      new Date(b.start_at).toLocaleString("th-TH", { timeZone: "Asia/Bangkok" }),
      b.customers?.[0]?.name || "",
      b.customers?.[0]?.phone || "",
      b.services?.[0]?.name || "",
      b.deposit_amount,
